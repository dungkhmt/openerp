...
const getActiveQuizOfSessionForStudentReducer = (
  state = initialState,
  action,
) => {
  // console.log('getActiveQuizOfSessionForStudentReducer: enter, action=' + JSON.stringify(action));
  switch (action.type) {
    case types.GET_ACTIVE_QUIZ_OF_SESSION_FOR_STUDENT:
      return {
        ...state,
        isFetching: true,
      };
    case types.GET_ACTIVE_QUIZ_OF_SESSION_FOR_STUDENT_SUCCESS:
    ...
    case types.GET_ACTIVE_QUIZ_OF_SESSION_FOR_STUDENT_FAILURE:
      return {
        ...state,
        isFetching: false,
...
export default getActiveQuizOfSessionForStudentReducer;
