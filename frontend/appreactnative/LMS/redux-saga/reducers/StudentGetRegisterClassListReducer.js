...
const studentGetRegisterClassListReducer = (state = initialState, action) => {
  // console.log('studentGetRegisterClassListReducer: enter, action=' + JSON.stringify(action));
  switch (action.type) {
    case types.STUDENT_GET_REGISTER_CLASS_LIST:
      return {
        ...state,
        isFetching: true,
      };
    case types.STUDENT_GET_REGISTER_CLASS_LIST_SUCCESS:
      // Merge registeredClasses
      var registeredClasses = state.studentRegisterClasses.registeredClasses;
      if (
        action.studentRegisterClasses.registeredClasses !== null &&
        action.studentRegisterClasses.registeredClasses !== undefined &&
        action.studentRegisterClasses.registeredClasses.length > 0
      ) {
        // TODO: Loai bo nhung id bi duplicate.
        registeredClasses =
          state.studentRegisterClasses.registeredClasses.concat(
            action.studentRegisterClasses.registeredClasses,
          );
      }

      // Merge content
      var oldList = state.studentRegisterClasses.list;
      var newList = oldList;
      if (
        action.studentRegisterClasses.page !== null &&
        action.studentRegisterClasses.page !== undefined &&
        action.studentRegisterClasses.page.content !== null &&
        action.studentRegisterClasses.page.content !== undefined &&
        action.studentRegisterClasses.page.content.length > 0
      ) {
        newList = action.studentRegisterClasses.page.content;
        newList.forEach(element => {
          if (registeredClasses.includes('' + element.id)) {
        ...
        // TODO: Truong hop trong new list va old list co element co id trung nhau, chi lay phan tu trong new list (xem nhu co du lieu moi hon so voi trong old list)
      }

      return {
        ...state,
        studentRegisterClasses: {
          semesterId: action.studentRegisterClasses.semesterId,
          list: newList,
          registeredClasses: registeredClasses,
        },
        isFetching: false,
      };
    case types.STUDENT_GET_REGISTER_CLASS_LIST_FAILURE:
      return {
        ...state,
        isFetching: false,
...
export default studentGetRegisterClassListReducer;
