...
import {getStudentClassSessionsAction} from '../../redux-saga/actions/GetStudentClassSessionsAction';
...
const StudentClassSessionsTab = ({route}) => {
  ...
  const loading = useSelector(state => state.getStudentClassSessionsReducer.isFetching);
  const sessionList = useSelector(
    state => state.getStudentClassSessionsReducer.sessionList,
  );

  useEffect(() => {
    console.log('StudentClassSessionsTab.useEffect: enter');
    dispatch(getStudentClassSessionsAction({studentClassId: studentClassId}));
  }, []);
    ...
    <SafeAreaView style={{flex: 1}}>
      <View style={{flex: 1}}>
        ...
        <FlatList
          ...
          data={sessionList}
          renderItem={({item}) => <Session data={item} />}
          keyExtractor={(item, index) => item.sessionId + index}
          onRefresh={() => dispatch(getStudentClassSessionsAction({studentClassId: studentClassId}))}
          refreshing={loading}
        />
      </View>
    </SafeAreaView>
