...
export const DrawText = React.memo(
  React.forwardRef(({ eventPointer, offset, tool, currentPage, onUpdateTool, whiteboardId, onDrawDone }, ref) => {
    ...
    useEffect(() => {
      socket.on(SOCKET_IO_EVENTS.ON_ADD_TEXT_END, ({ data, currentDrawPage, currentWhiteboardId }) => {
        ...
        if (Number(currentDrawPage) === Number(currentPage)) {
          ...
          if (
            (ref?.getLayers().length > 0 && ref?.getLayers()[4]?.getChildren().length / 2 !== data.length) ||
      ...
      <Layer>
        {annotations.map((value) => (
          <React.Fragment key={value.id}>
            <Text
              ref={(el) => (textRef.current[value.id] = el)}
              x={value.x}
              y={value.y}
              width={value.width}
              draggable={tool !== TOOL.POINTER ? false : true}
              ...
              align={value.align}
              onClick={(e) => onClickText(e.target, value)}
              onDblClick={(e) => onDblClickTextRect(e.target, value)}
              onDblTap={(e) => onDblClickTextRect(e.target, value)}
              onDragEnd={(e) => onDragTextEnd(e.target, value)}
              onTransformEnd={(e) => onTextTransformEnd(e.target, value)}
            />
            <Transformer
              ref={(el) => (transformRef.current[value.id] = el)}
              boundBoxFunc={function (_, newBox) {
                newBox.width = Math.max(30, newBox.width)
                return newBox
              }}
