...
import {CardActions, Button, Card, CardContent, FormControl, FormLabel, Stack, TextField} from "@mui/material";
...
function QuizTestEdit() {
  ...
  const [questionStatementViewTypeId, setQuestionStatementViewTypeId] = useState(null);
  const [listQuestionStatementViewTypeId, setListQuestionStatementViewTypeId] = useState([]);

  const [participantQuizGroupAssignmentMode, setParticipantQuizGroupAssignmentMode] = useState(null);
  const [listParticipantQuizGroupAssignmentMode, setListParticipantQuizGroupAssignmentMode,] = useState([]);
  ...
  function getListParticipantQuizGroupAssignmentMode() {
    let successHandler = (res) => setListParticipantQuizGroupAssignmentMode(res.data);
    request("GET", "get-list-participant-quizgroup-assignment-mode", successHandler);
  }

  function getListQuizTestStatusIds() {
    request("GET", "get-list-quiz-test-status-ids", (res) => setListStatusIds(res.data));
  }

  function getListQuestionStatementViewTypeId() {
    request("GET", "get-list-question-statement-view-type-id",
      (res) => setListQuestionStatementViewTypeId(res.data) );
  }
  function getListQuizTestViewTypeId() {
    request("GET", "get-list-quiz-test-view-type-id", (res) => setListViewTypeIds(res.data));
  }
  ...
  async function getQuizTestDetail() {
    request("GET", "get-quiz-test?testId=" + testId, (res) => {
        ...
        setParticipantQuizGroupAssignmentMode(res.data.participantQuizGroupAssignmentMode);
    });
  }

  function updateQuizTest() {
    let updatedInfo = {
      testId, scheduleDate: selectedDate, duration,
      statusId, questionStatementViewTypeId,
      viewTypeId, participantQuizGroupAssignmentMode,
    };

    let successHandler = (res) => successNoti("Cập nhật quiz test thành công", true);
    let errorHandlers = {
      onError: (error) => errorNoti("Đã xảy ra lỗi khi cập nhật quiz test", true),
    };
    request("POST", "update-quiz-test", successHandler, errorHandlers, updatedInfo);
  ...
  return (
    <Grid container spacing={1} justify="center">
      <Card style={{ padding: "3% 10% 3% 10%", minWidth: "1024px" }}>
        ...
        <CardHeader title={ <span>Chỉnh sửa quiz test <em>{quizTest?.testName}</em></span> }/>
        ...
        <CardContent>
          <Stack spacing={2}>
            ...
            <Stack direction="row" spacing={2}>
              <SelectItem label="Trạng thái"
                          style={{ width: '100%'}}
                          value={statusId}
                          options={quizTestStatusOptions}
                          onChange={value => setStatusId(value)}/>
              <SelectItem label="Dạng hiển thị danh sách câu hỏi"
                          style={{ width: '100%' }}
                          value={viewTypeId}
            ...
            <Stack direction="row" spacing={2}>
              <SelectItem label="Participant-quiz group assignment mode"
                          style={{ width: '100%'}}
                          value={participantQuizGroupAssignmentMode}
                          options={participantQuizGroupAssignmentModeOptions}
                          onChange={value => setParticipantQuizGroupAssignmentMode(value)}/>
              <SelectItem label="Question statement view"
                          style={{ width: '100%' }}
