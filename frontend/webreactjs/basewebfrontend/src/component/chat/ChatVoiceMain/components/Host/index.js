...
export default function Host() {
  ...
  const { data: invitedFriends } = useGetInvitedFriends({ meetId: modalContentData?.meetId });
  ...
  const joinMeet = () => {
    history.push({
      pathname: `main/${modalContentData.meetId}`
    });
  }
  const renderModalContent = useMemo(() => {
    ...
    return (
      <Box sx={styleModal} className='host-modal'>
        ...
        <div className='host-modal-invited-friends-title'>Danh sách người được mới</div>
        <Grid className='host-modal-invited-friends' container spacing={2}>
          {invitedFriends?.content?.length === 0 && <Grid className='host-modal-no-invite' item xs={12}>Hiện tại chưa có ai được mời</Grid>}
          {invitedFriends?.content?.map((friendName, index) => (<Grid className='host-modal-invited-friend' key={index} item xs={6}>&#9830; {friendName}</Grid>))}
        </Grid>
        ...
        <Grid container spacing={2}>
          ...
          <Grid item xs={6}>
            <PrimaryButton onClick={joinMeet}>
              Tham gia ngay
            </PrimaryButton>
          </Grid>
        </Grid>
  ...
  }, [modalContentData, invitedFriends]);

  return (
    <>
      <ScheduleMeet refetchOwnedMeets={refetch} />
      <ListMeet title='Danh sách cuộc họp của bạn' listMeet={listMeet || []} onClickMeet={handleClickMeet} />
      <ButtonMeetNow />
      <Modal
