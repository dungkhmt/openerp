...
import useStyles from "./UpdateWarehouse.style";
...
export default function UpdateWarehouse() {
  ...
  const [shelfVariants, setShelfVariant] =  useState([]);
  let { id } = useParams();
  ...
  const getAllShelfVariantInFacility = () => {
    request(
      "get",
      `admin/wms/warehouse/products/shelf/facility/${id}`,
      (res) => {
        setShelfVariant(res.data);
      },
      {
        onerror: (res) => {
          console.log("getWarehouseDetail, error ")
  ...
  const handleAddShelf = () => {
    setListShelf([...listShelf, { shelfId: "", x: "", y: "", width: "", lenght: "", num: "" }])
  }

  let removeFormFields = (i) => {

    const find = shelfVariants.find(item => item.shelfId === listShelf[i].shelfId );
    if(find && find.quantity > 0){
      errorNoti(
        "Không thể xóa kệ hàng chứa sản phẩm"
      );
    }else{
  ...
  let submitForm = (data) => {
    ...
    request(
      ...
      (res) => {
        ...
        history.push(`${path.replace('/update/:id', '')}/${id}`);
      },
      { 401: () => { } },
      data
    );
  ...
  const changeWareHouseWidth = (e) =>{
    setWarehouseData({...warehouesData,facilityWidth: e.target.value })
  }
                    ...
                    <Grid item xs={6}>
                      <Box className={classes.inputWrap}>
                        ...
                        <TextField
                          ...
                          name="name"
                          onChange={(e) => setWarehouseData({...warehouesData,name: e.target.value })}
                          value= {warehouesData.name}
                          error={!!errors.name}
                    ...
                    <Grid item xs={6}>
                      <Box className={classes.inputWrap}>
                        ...
                        <TextField
                          ...
                          name="code"
                          value= {warehouesData.code}
                          onChange={(e) => setWarehouseData({...warehouesData,code: e.target.value })}
                          error={!!errors.code}
                    ...
                    <Grid item xs={12}>
                      <Box className={classes.inputWrap}>
                        ...
                        <TextField
                          ...
                          name="address"
                          value= {warehouesData.address}
                          onChange={(e) => setWarehouseData({...warehouesData,address: e.target.value })}
                    ...
                    <Grid item xs={12}>
                      <Box className={classes.inputWrap}>
                        ...
                        <OutlinedInput
                          ...
                          name="facilityWidth"
                          value= {warehouesData.facilityWidth}
                          onChange={(e) => changeWareHouseWidth(e)}
                    ...
                    <Grid item xs={12}>
                      <Box className={classes.inputWrap}>
                        ...
                        <OutlinedInput
                          fullWidth
                          name="facilityLenght"
                          value= {warehouesData.facilityLenght}
                          inputRef={register({ required: false })}
                          className={classes.settingInput}
                          onChange={(e) => setWarehouseData({...warehouesData,facilityLenght: e.target.value })}
                          endAdornment={<InputAdornment position="end">{`(mét)`}</InputAdornment>}
                        />
                      </Box>
                    </Grid>
