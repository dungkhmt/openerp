...
function WarehouseDetail() {
  ...
  const stageCanvasRef = useCallback(
    (node) => {
      if (node) {
        setHeight(node.getBoundingClientRect().height);
        setWidth(node.getBoundingClientRect().width);
        setWarehouseHeight(
          (node.getBoundingClientRect().width * warehouesData.facilityLenght) /
            warehouesData.facilityWidth
        );
        setScale(
          node.getBoundingClientRect().width / warehouesData.facilityWidth
        );
      }
    },
    [warehouesData]
  ...
  const handleImport = () => {
    history.push(
      `${path.replace("/warehouse/:id", "/inventory/import/create")}`
    );
  };
  const handleExit = () => {
  ...
  return (
    <Fragment>
      ...
      >
        <Grid>
          <Typography variant="h5">Kho : {warehouesData.name}</Typography>
        </Grid>
              ...
              <Grid container className={classes.grid}>
                ...
                <Grid item md={8} sm={8} xs={8} container direction="column">
                  ...
                  <Typography className={classes.label}>
                    ...
                    {warehouesData.facilityWidth} m
                  </Typography>
                  <Typography className={classes.label}>
                    <b style={{ marginRight: 20 }}>:</b>{" "}
                    {warehouesData.facilityLenght} m
                  </Typography>
                </Grid>
              </Grid>

              <Box className={classes.desLabelWrap}>
                <Typography className={classes.desLabel} variant="h5">
                  Danh sách sản phẩm
                </Typography>
              </Box>
                      ...
                      <Stage width={width} height={height}>
                        <Layer>
                          <Rect
                            ...
                            cornerRadius={3}
                          />
                          {listShelf.map((data) => warehouseBox(data, scale))}
                        </Layer>
                      </Stage>
                      {shelf && pos && (
                        <Typography
              ...
              <Box style={{ padding: 10, width: "33.33%" }}>
                <Box className={classes.shelfDetail}>
                  <Box className={classes.desLabelWrap} style={{ margin: 0 }}>
                    <Typography className={classes.desLabel} variant="h5">
                      Danh sách sản phẩm : kệ hàng số {facilityNum}
                    </Typography>
                  </Box>
