...
public class SearchOptimumByLocalSearch {
	...
	public void explore(int maxIters, int maxTime, int maxStable, String outputfile){
		...
		while(it++ < maxIters && System.currentTimeMillis()/1000 - t0 < maxTime){
			System.out.println(System.currentTimeMillis()/1000 - t0);
			if(cnt > maxStable){
				allRemoval();
				greedyInsertion();
				cnt = 0;
...
//					|| (nR == best_solution.get_nbReject() && tcs.objective.getValue() < best_solution.get_cost()))
			...
			switch(op){
				...
				case 4: exploreForOrOptMove();break;
				case 5: exploreForThreePointMove(); break;
				case 6: exploreForThreeOptMove5(); break;
			}
...
//					|| (nR == best_solution.get_nbReject() && new_cost < best_solution.get_cost())){
	...
	private void exploreForThreePointMove(){
		if(tcs.getNbUsedTrucks() == 1)
			return;
		Random ran = new Random();
		Point pickup1 = null;
		...
		if(pickup1 != null && pickup2 != null)
			twoRequestExchangeRoute(pickup1, pickup2);
		exploreForOneRequestMoveWithGreedyInsertion();
	}
	
	private void exploreForOrOptMove(){
		if(tcs.getNbUsedTrucks() == 1)
			return;
		Random ran = new Random();
		Point pickup1 = null;
...
//				System.out.println("sad");
	...
	private void exploreForThreeOptMove5(){
		if(tcs.getNbUsedTrucks() == 1)
			return;
		Random ran = new Random();
