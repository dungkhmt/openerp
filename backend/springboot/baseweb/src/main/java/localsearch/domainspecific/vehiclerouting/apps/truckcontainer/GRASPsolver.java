...
//					continue;
	...
	public void insertMoocToRoutes(int r){
		Point st = XR.getStartingPointOfRoute(r);
		Point et = XR.getTerminatingPointOfRoute(r);
		Point stMooc = null;
		Point enMooc = null;
		for(Point p = XR.next(st); p != XR.getTerminatingPointOfRoute(r); p = XR.next(p)){
			if(accMoocInvr.getSumWeights(XR.prev(p)) <= 0){
...
//		}
	...
	public ArrayList<Double> computeProb(ArrayList<Integer> index, ArrayList<Integer> rank){
		...
		for(int i = 0; i < index.size(); i++){
			double p = (double)bias.get(i) / sum_bias;
			prob.add(p);
		}
		return prob;
	}
	
	public void greedy_randomized_contruction(){
		int nPoints = pickup2Delivery.size();
		...
		for(int r : routeIdx){
			...
			while(true){
				...
				for(int i = 0; i < nPoints; i++){
					...
					int groupId = point2Group.get(pickup);
					if(group2marked.get(groupId) == 1 
						|| XR.route(pickup) != Constants.NULL_POINT
...
//								removeMoocOnRoutes(r);
				...
				if(pick_cand.size() > 0){
					...
					for(int i = 0; i < obj_cand.size(); i++){
						double cost = obj_cand.get(i);
						if(cost <= (min_cost + this.alpha*(max_cost - min_cost))){
							mgr.performAddTwoPoints(pick_cand.get(i), pre_pick_cand.get(i),
									del_cand.get(i), pre_del_cand.get(i));
							insertMoocToRoutes(r);
							if(S.violations() == 0)
								index_rcl.add(i);
							mgr.performRemoveTwoPoints(pick_cand.get(i), del_cand.get(i));
							removeMoocOnRoutes(r);
						}
					}
					ArrayList<Double> prob = computeProb(index_rcl, rank_cand);
					...
					mgr.performAddTwoPoints(pick_cand.get(rand), pre_pick_cand.get(rand),
							del_cand.get(rand), pre_del_cand.get(rand));
					int groupTruck = point2Group.get(st);
					group2marked.put(groupTruck, 1);
					group2marked.put(group_cand.get(rand), 1);

					rejectPickupPoints.remove(pick_cand.get(rand));
					rejectDeliveryPoints.remove(del_cand.get(rand));
					nbR++;
				}
				else
					break;
	...
	public void greedy_randomized_contruction_req(){
		int nPoints = pickup2Delivery.size();
		int nbRoutes = XR.getNbRoutes();
		int nbR = 0;
		for(int i = 0; i < nPoints; i++){
			...
			if(group2marked.get(groupId) == 1 
				...
				if(truck_cand.size() > 0){
					...
					for(int t = 0; t < obj_cand.size(); t++){
						double cost = obj_cand.get(t);
						if(cost <= (min_cost + this.alpha*(max_cost - min_cost))){
							mgr.performAddTwoPoints(pickup, pre_pick_cand.get(t),
									delivery, pre_del_cand.get(t));
							insertMoocToRoutes(truck_cand.get(t));
							if(S.violations() == 0)
								index_rcl.add(t);
							mgr.performRemoveTwoPoints(pickup, delivery);
							removeMoocOnRoutes(truck_cand.get(t));
						}
					}
					ArrayList<Double> prob = computeProb(index_rcl, rank_cand);
					DistributedRandomNumberGenerator g = new DistributedRandomNumberGenerator(index_rcl, prob);
					int rand = g.getDistributedRandomNumber();
					mgr.performAddTwoPoints(pickup, pre_pick_cand.get(rand),
							delivery, pre_del_cand.get(rand));
					int groupTruck = point2Group.get(XR.getStartingPointOfRoute(truck_cand.get(rand)));
					group2marked.put(groupTruck, 1);
	...
	public void greedy_randomized_contruction_req_shuffle(){
		int nPoints = pickupPoints.size();
		int nbRoutes = XR.getNbRoutes();
		int nbR = 0;
		List<Point> consideredPoint = new ArrayList<Point>(pickupPoints);
	...
	public void adaptiveSearchOperators(String outputfile){	
		...
		lower_removal = (int) (0.05*nRequest);
		upper_removal = (int) (0.35*nRequest);
		sigma1 = 3;
		sigma2 = -1;
		sigma3 = -5;
		rp = 0.1;
    	...
    	ALNSforGRASP opt = new ALNSforGRASP(this);
    	
		double best_cost = objective.getValue();
		int best_nb_reject_points = getNbRejectedRequests();
		int best_nbTrucks = getNbUsedTrucks();
		TruckContainerSolution best_solution = new TruckContainerSolution(XR, rejectPickupPoints,
				rejectDeliveryPoints, best_cost, best_nbTrucks, best_nb_reject_points, point2Group, group2marked);

		double start_search_time = System.currentTimeMillis();

		while( (System.currentTimeMillis()-start_search_time) < timeLimitALNS 
				&& it++ < nIterALNS){
			//System.out.println("nb of iterator: " + it);
			double current_cost = objective.getValue();
			int current_nbTrucks = getNbUsedTrucks();
			int current_nb_reject_points = getNbRejectedRequests();

			TruckContainerSolution current_solution = new TruckContainerSolution(XR, rejectPickupPoints, 
				rejectDeliveryPoints, current_cost, current_nbTrucks, current_nb_reject_points, point2Group, group2marked);
			...
			switch(i_selected_insertion){
				...
				case 1: opt.first_possible_insertion(); break;
				case 2: opt.regret_n_insertion(2); break;
				case 3: opt.greedy_randomized_contruction_req_shuffle(); break;
			...
			int new_nb_reject_points = getNbRejectedRequests();
			...
			if( new_nb_reject_points < current_nb_reject_points
				...
				if(new_nb_reject_points < best_nb_reject_points
					...
					best_cost = new_cost;
					best_nb_reject_points = new_nb_reject_points;
					best_nbTrucks = new_nbTrucks;
...
//						+ ", cost = " + awm.getDistance(x, XR.next(x)));
	...
	public void GRASP(String outputfile){
		int it = 0;

		System.out.println("start...");
		greedy_randomized_contruction_req_shuffle();
		double best_cost = objective.getValue();
		int best_nbTrucks = getNbUsedTrucks();
		int best_nb_reject_points = getNbRejectedRequests();
		...
		System.out.println("initial cost = " + best_cost + ", nbReject = " + best_nb_reject_points + ", nbTrucks = " + best_nbTrucks);
		...
		try{
			...
			FileOutputStream write = new FileOutputStream(outputfile, true);
			PrintWriter fo = new PrintWriter(write);
			fo.println("time limit = " + timeLimit + ", nbIters = " + nIter);
			fo.println("#Request = " + nRequest);
			fo.println("iter=====alpha=====time=====cost=====nbReject=====nbTrucks");
			fo.println("0 " + alpha + " " + System.currentTimeMillis()/1000 + " " + best_cost + " " + best_nb_reject_points + " " + best_nbTrucks);
			fo.close();
		}catch(Exception e){
			System.out.println(e);
		}
		...
		while((System.currentTimeMillis()-start_search_time) < timeLimit && it++ < nIter){
			System.out.println("it = " + it + ", time = " + (System.currentTimeMillis()-start_search_time));
			removeAllMoocFromRoutes();
			allRemoval();
			greedy_randomized_contruction_req_shuffle();
			System.out.println("greedy cost = " + objective.getValue() + ", nbReject = " + getNbRejectedRequests() + ", nbTrucks = " + getNbUsedTrucks());
			...
			System.out.println("alns cost = " + new_cost + ", nbReject = " + new_nb_reject_points + ", nbTrucks = " + new_nbTrucks);
			...
			if( new_nb_reject_points < best_nb_reject_points
				...
				best_solution = new TruckContainerSolution(XR, rejectPickupPoints, rejectDeliveryPoints,
						new_cost, new_nbTrucks, new_nb_reject_points, point2Group, group2marked);
				System.out.println("best cost = " + best_cost + ", nbReject = " + best_nb_reject_points + ", nbTrucks = " + best_nbTrucks);
				...
				try{
					...
					fo.println(it + " " + alpha + " "
						...
						+ best_cost + " " + best_nb_reject_points + " " + best_nbTrucks);
					fo.close();
				}catch(Exception e){
					System.out.println(e);
				}
	...
	public static void main(String[] args){
		...
		String fileName = "random-200reqs-RealLoc-0";
		String dataFileName = dir + "input/" + fileName + ".txt";
		for(int i = 1; i < 9; i++){
			String outputGRASPfileTxt = dir + "output/GRASP/heuristic/It-" + i + "-GRASP-" + fileName + ".txt";
			String outputGRASPfileJson = dir + "output/GRASP/heuristic/It-" + i + "-GRASP-" + fileName + ".json";
			...
			try{
				FileOutputStream write = new FileOutputStream(outputGRASPfileTxt);
				PrintWriter fo = new PrintWriter(write);
			...
			solver.GRASP(outputGRASPfileTxt);
			
			solver.printSolution(outputGRASPfileTxt);
			
			Gson g = new Gson();
			TruckMoocContainerOutputJson solution = solver.createFormatedSolution();
			try{
				String out = g.toJson(solution);
				BufferedWriter writer = new BufferedWriter(new FileWriter(outputGRASPfileJson));
			    writer.write(out);
