...
public interface TaskExecutionRepository
    ...
    @Query(value = "SELECT e.* FROM backlog_task_execution e WHERE e.task_id = :taskId", nativeQuery = true)
    List<TaskExecution> findByTaskId(@Param("taskId") UUID taskId);

    @Query(value = "SELECT DISTINCT DATE(e.created_stamp) FROM backlog_task_execution e WHERE e.project_id = :projectId",
           nativeQuery = true)
    List<Object[]> getAllDistinctDay(@Param("projectId") UUID projectId);

    @Query(value = "select e.* from backlog_task_execution e\n" +
                   " where e.project_id = :projectId AND ( e.created_stamp BETWEEN :startDate AND :endDate )", nativeQuery = true)
    List<TaskExecution> getAllTaskExecutionByDate(@Param("startDate") Date startDate, @Param("endDate") Date endDate, @Param("projectId") UUID projectId);
}
